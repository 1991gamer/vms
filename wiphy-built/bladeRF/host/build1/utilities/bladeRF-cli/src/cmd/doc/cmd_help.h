 ...
 *
 * Copyright (C) 2014-2025 Nuand LLC
 *
 ...
 *
 * Last generated: Wed Jan  1 08:29:05 PM EST 2025 by spatula@my
 */
...
#define CLI_CMD_HELPTEXT_calibrate \
  ...
  "    to the AGC&rsquo;s base gain value before running\n" \
  "    calibrate table dc.\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_flash_image \
  ...
  "\n" \
  "Print a flash image&rsquo;s metadata or create a new flash image. When\n" \
...
#define CLI_CMD_HELPTEXT_flash_init_cal \
  ...
  "    no data will be written to the device&rsquo;s flash.\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_fw_log \
  ...
  "\n" \
  "Read the contents of the device&rsquo;s firmware log and write it to\n" \
  "the specified file. If no filename is specified, the log content is\n" \
  "written to stdout.\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_generate \
  ...
  "-   cw\n" \
  "\n" \
  "    Generate a complex tone with a period relative to system sampling\n" \
  "    rate\n" \
  "\n" \
  "    period indicates the number of samples between 2*M_PI. For\n" \
  "    example, cw of 4 would generate a complex tone at +4/F_s. With a\n" \
  "    sampling rate of 20MSPS, the complex tone would be generated at\n" \
  "    +5MHz. A negative period indicates negative frequency.\n" \
  "\n" \
  "-   prn\n" \
  "\n" \
  ...
  "Examples:\n" \
  "\n" \
  "-   To generate a CSV for a complex tone at -F_s/4 with a full scale\n" \
  "    swing of -500 to 500:\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_load \
  ...
  "\n" \
  "Load an FPGA bitstream or program the FX3&rsquo;s SPI flash.\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_mimo \
  ...
  "\"print/set smb_mode\". For usage text, run: &ldquo;set smb_mode&rdquo;\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_recover \
  ...
  "device&rsquo;s RAM, users should open the device with the\n" \
  "&ldquo;open&rdquo; command, and write the firmware to flash via\n" \
  "&ldquo;load fx3 <firmware file>&rdquo;\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_rx \
  ...
  "    (e.g.&nbsp;/tmp, /dev/shm), if space allows. For larger captures\n" \
  "    at higher sample rates, consider using an SSD instead of a HDD.\n" \
...
#define CLI_CMD_HELPTEXT_trigger \
  ...
  "triggers. When and <tx | rx> and supplied, the specified trigger is\n" \
  "printed.\n" \
  "\n" \
  ...
  "\n" \
  "    The operation will stall until the triggers fire. As such,\n" \
  "    sufficiently large timeouts should be used to allow the trigger\n" \
  "    signal to be emitted by the master and received by the slaves\n" \
  "    prior to libbladeRF returning BLADERF_ERR_TIMEOUT.\n" \
  "\n" \
  "4.  Set fire-request on master trigger\n" \
  "\n" \
  "    All devices will synchronously start transmitting or receiving\n" \
  "    data.\n" \
  "\n" \
...
#define CLI_CMD_HELPTEXT_tx \
  ...
  "          timeout Data stream timeout. With no suffix, the default unit\n" \
  "                  is ms. The default value is 1000 ms (1 s). Valid\n" \
  "                  suffixes are &lsquo;ms&rsquo; and &lsquo;s&rsquo;.\n" \
  "\n" \
  "          channel Comma-delimited list of physical RF channels to use\n" \
  "  ---------------------------------------------------------------------\n" \
  "\n" \
