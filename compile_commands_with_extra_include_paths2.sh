...
compile_commands() {
    ...
    #cat $OUTPUT
    DIR_JSON=$(jq -nR "{directory:\"$PWD\"}")
    MAIN_JSON=$(make \
        | grep -wE 'gcc|g\+\+' \
        | grep -w '\-c' \
        | jq -nR 'inputs | {arguments:split(" "), file:match(" [^ ]+\\.c[cp]*").string[1:]}')
    MERGE_JSON=$(echo "$MAIN_JSON $DIR_JSON" | jq -s add)
    echo "[$MERGE_JSON]"
}
...
#    jq ".arguments += [$COMMA_SEPARATED_QUOTED_INCLUDE_ARGS]"
compile_commands | \
